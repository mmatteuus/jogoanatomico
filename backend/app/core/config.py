...
class Settings(BaseSettings):
    ...
    _DRIVER_MAP: ClassVar[dict[str, tuple[str, str]]] = {
    ...
    def _resolve_driver(self, url: str, *, async_driver: bool) -> str:
        sa_url: URL = make_url(url)
        backend = sa_url.get_backend_name()
        driver_options = self._DRIVER_MAP.get(backend)
        if not driver_options:
            return url
        target_driver = driver_options[0 if async_driver else 1]
        return self._ensure_driver(url, target_driver)

    @property
    def async_database_url(self) -> str:
        return self._resolve_driver(self.database_url, async_driver=True)
    ...
    def sync_database_url_with_driver(self) -> str:
        return self._resolve_driver(self.sync_database_url_value, async_driver=False)
